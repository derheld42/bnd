...
public class ProjectTest extends TestCase {
	...
	public static void testBndRawMacro() throws Exception {
		...
		String f = p.getReplacer()._raw(new String[] {"raw", "-runbundles"});
	...
	public static void testBndRawMacro2() throws Exception {
		...
		assertEquals("osgi.identity;filter:='(osgi.identity=variable)',osgi.identity;filter:='(osgi.identity=variable2)',osgi.identity;filter:='(osgi.identity=b)',osgi.identity;filter:='(osgi.identity=c)'", string);
		
		f = p.getReplacer()._raw(new String[] {"raw", "-runrequires"});
